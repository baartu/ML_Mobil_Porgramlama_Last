import 'package:deneme/ekranlar/profil.dart';
import 'package:flutter/material.dart';

import 'package:firebase_auth/firebase_auth.dart';
import 'package:cloud_firestore/cloud_firestore.dart';

class GuncellemeSayfasi extends StatefulWidget {
  const GuncellemeSayfasi({super.key});

  @override
  State<GuncellemeSayfasi> createState() => _ProfilSayfasiState();
}

class _ProfilSayfasiState extends State<GuncellemeSayfasi> {
  final TextEditingController _adController = TextEditingController();
  final TextEditingController _emailController = TextEditingController();
  final TextEditingController _sifreController = TextEditingController();
  bool _sifreGizli = true;
  @override
  void initState() {
    super.initState();
    _verileriGetir(); // Firestore'dan verileri çek
  }

  @override
  void dispose() {
    _sifreController.dispose();
    super.dispose();
  }

  Future<void> _verileriGuncelle() async {
    User? user = FirebaseAuth.instance.currentUser;

    if (user != null) {
      try {
        // En az bir alanın dolu olması kontrolü
        if (_adController.text.isEmpty &&
            _emailController.text.isEmpty &&
            _sifreController.text.isEmpty) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('En az bir alanı doldurun!')),
          );
          return;
        }

        // Email formatı kontrolü (eğer email değiştirilecekse)
        if (_emailController.text.isNotEmpty &&
            !RegExp(r'^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$')
                .hasMatch(_emailController.text)) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Geçerli bir e-posta adresi girin!')),
          );
          return;
        }

        // Şifre uzunluk kontrolü (eğer şifre değiştirilecekse)
        if (_sifreController.text.isNotEmpty &&
            _sifreController.text.length < 6) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Şifre en az 6 karakter olmalıdır!')),
          );
          return;
        }

        // Firestore güncelleme için map oluştur
        Map<String, dynamic> updateData = {};

        // Sadece değişen alanları ekle
        if (_adController.text.isNotEmpty) {
          updateData['ad'] = _adController.text;
        }
        if (_emailController.text.isNotEmpty) {
          updateData['email'] = _emailController.text;
        }

        // Firestore'u güncelle (eğer güncellenecek veri varsa)
        if (updateData.isNotEmpty) {
          try {
            await FirebaseFirestore.instance
                .collection('users')
                .doc(user.uid)
                .update({
              ...updateData,
              'guncellemeTarihi': FieldValue.serverTimestamp(),
            });
          } catch (firestoreError) {
            print('Firestore güncelleme hatası: $firestoreError');
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('Veritabanı güncellemesi başarısız!'),
                backgroundColor: Colors.red,
              ),
            );
            return;
          }
        }

        // FirebaseAuth güncellemeleri
        try {
          // Email güncelleme (eğer değiştirilecekse)
          if (_emailController.text.isNotEmpty &&
              user.email != _emailController.text) {
            // Önce mevcut email'in doğrulanmış olup olmadığını kontrol et
            if (!user.emailVerified) {
              // Email doğrulama maili gönder
              await user.sendEmailVerification();

              if (mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text(
                        'Lütfen mevcut e-posta adresinizi doğrulayın. Doğrulama maili gönderildi.'),
                    backgroundColor: Colors.orange,
                  ),
                );
              }
              return;
            }

            // Email doğrulanmışsa güncelle
            await user.updateEmail(_emailController.text);
          }

          // Şifre güncelleme (eğer değiştirilecekse)
          if (_sifreController.text.isNotEmpty) {
            await user.updatePassword(_sifreController.text);
          }
        } catch (authError) {
          print('Firebase Auth güncelleme hatası: $authError');
          if (authError is FirebaseAuthException) {
            String errorMessage;
            switch (authError.code) {
              case 'email-already-in-use':
                errorMessage = 'Bu e-posta adresi zaten kullanımda!';
                break;
              case 'invalid-email':
                errorMessage = 'Geçersiz e-posta adresi!';
                break;
              case 'weak-password':
                errorMessage = 'Şifre çok zayıf!';
                break;
              case 'requires-recent-login':
                errorMessage =
                    'Güvenlik nedeniyle tekrar giriş yapmanız gerekiyor!';
                break;
              case 'operation-not-allowed':
                errorMessage =
                    'Lütfen önce mevcut e-posta adresinizi doğrulayın!';
                break;
              default:
                errorMessage = 'Güncelleme başarısız: ${authError.message}';
            }
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(errorMessage),
                backgroundColor: Colors.red,
              ),
            );
            return;
          }
        }

        // Başarı mesajı göster
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Bilgiler başarıyla güncellendi!'),
              backgroundColor: Colors.green,
            ),
          );

          // Profil sayfasına yönlendir
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(builder: (context) => const ProfilSayfasi()),
          );
        }
      } catch (error) {
        print('Genel Hata: $error');
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Bir hata oluştu: $error'),
              backgroundColor: Colors.red,
            ),
          );
        }
      }
    }
  }

  Future<void> _verileriGetir() async {
    User? user = FirebaseAuth.instance.currentUser;
    if (user != null) {
      DocumentSnapshot userDoc = await FirebaseFirestore.instance
          .collection('users')
          .doc(user.uid)
          .get();

      if (userDoc.exists) {
        Map<String, dynamic> data = userDoc.data() as Map<String, dynamic>;

        setState(() {
          _adController.text = data['ad'] ?? '';
          _emailController.text = data['email'] ?? '';
          // Şifre alanını boş bırakıyoruz çünkü güvenlik nedeniyle Firestore'da saklanmamalı
          _sifreController.text = '';
        });
      }
    }
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      body: Container(
        decoration: BoxDecoration(
          gradient: LinearGradient(
            begin: Alignment.topCenter,
            end: Alignment.bottomCenter,
            colors: [
              Colors.blue.shade100,
              Colors.white,
            ],
          ),
        ),
        child: SafeArea(
          child: Column(
            children: [
              Container(
                padding:
                    const EdgeInsets.symmetric(horizontal: 20, vertical: 10),
                child: Row(
                  children: [
                    IconButton(
                      icon: const Icon(Icons.arrow_back_ios,
                          color: Colors.blueGrey),
                      onPressed: () => Navigator.pop(context),
                    ),
                    const Text(
                      "Bilgileri Güncelle",
                      style: TextStyle(
                        fontSize: 24,
                        fontWeight: FontWeight.bold,
                        color: Colors.blueGrey,
                      ),
                    ),
                  ],
                ),
              ),
              Expanded(
                child: Center(
                  child: SingleChildScrollView(
                    child: Padding(
                      padding: const EdgeInsets.all(20.0),
                      child: Container(
                        padding: const EdgeInsets.all(20),
                        decoration: BoxDecoration(
                          color: Colors.white,
                          borderRadius: BorderRadius.circular(20),
                          boxShadow: [
                            BoxShadow(
                              color: Colors.blue.withOpacity(0.1),
                              blurRadius: 10,
                              offset: const Offset(0, 5),
                            ),
                          ],
                        ),
                        child: Column(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            Container(
                              padding: const EdgeInsets.all(15),
                              decoration: BoxDecoration(
                                color: Colors.blue.shade50,
                                shape: BoxShape.circle,
                              ),
                              child: const Icon(
                                Icons.edit_note,
                                size: 80,
                                color: Colors.blueGrey,
                              ),
                            ),
                            const SizedBox(height: 20),
                            Text(
                              "Bilgileri Düzenle",
                              style: TextStyle(
                                fontSize: 24,
                                fontWeight: FontWeight.bold,
                                color: Colors.blueGrey[800],
                              ),
                            ),
                            const SizedBox(height: 30),
                            Container(
                              decoration: BoxDecoration(
                                color: Colors.blue.shade50,
                                borderRadius: BorderRadius.circular(15),
                              ),
                              child: TextField(
                                controller: _adController,
                                decoration: InputDecoration(
                                  labelText: "Ad Soyad",
                                  labelStyle:
                                      TextStyle(color: Colors.blueGrey[700]),
                                  border: OutlineInputBorder(
                                    borderRadius: BorderRadius.circular(15),
                                    borderSide: BorderSide.none,
                                  ),
                                  prefixIcon: Icon(Icons.person,
                                      color: Colors.blueGrey[700]),
                                  filled: true,
                                  fillColor: Colors.transparent,
                                ),
                              ),
                            ),
                            const SizedBox(height: 20),
                            Container(
                              decoration: BoxDecoration(
                                color: Colors.blue.shade50,
                                borderRadius: BorderRadius.circular(15),
                              ),
                              child: TextField(
                                controller: _emailController,
                                decoration: InputDecoration(
                                  labelText: "E-Posta",
                                  labelStyle:
                                      TextStyle(color: Colors.blueGrey[700]),
                                  border: OutlineInputBorder(
                                    borderRadius: BorderRadius.circular(15),
                                    borderSide: BorderSide.none,
                                  ),
                                  prefixIcon: Icon(Icons.email,
                                      color: Colors.blueGrey[700]),
                                  filled: true,
                                  fillColor: Colors.transparent,
                                ),
                              ),
                            ),
                            const SizedBox(height: 20),
                            Container(
                              decoration: BoxDecoration(
                                color: Colors.blue.shade50,
                                borderRadius: BorderRadius.circular(15),
                              ),
                              child: TextField(
                                controller: _sifreController,
                                obscureText: _sifreGizli,
                                decoration: InputDecoration(
                                  labelText: "Şifre",
                                  labelStyle:
                                      TextStyle(color: Colors.blueGrey[700]),
                                  border: OutlineInputBorder(
                                    borderRadius: BorderRadius.circular(15),
                                    borderSide: BorderSide.none,
                                  ),
                                  prefixIcon: Icon(Icons.lock,
                                      color: Colors.blueGrey[700]),
                                  suffixIcon: IconButton(
                                    icon: Icon(
                                      _sifreGizli
                                          ? Icons.visibility_off
                                          : Icons.visibility,
                                      color: Colors.blueGrey[700],
                                    ),
                                    onPressed: () {
                                      setState(() {
                                        _sifreGizli = !_sifreGizli;
                                      });
                                    },
                                  ),
                                  filled: true,
                                  fillColor: Colors.transparent,
                                ),
                              ),
                            ),
                            const SizedBox(height: 40),
                            Container(
                              width: double.infinity,
                              height: 55,
                              decoration: BoxDecoration(
                                gradient: LinearGradient(
                                  colors: [
                                    Colors.blueGrey.shade700,
                                    Colors.blueGrey.shade500
                                  ],
                                ),
                                borderRadius: BorderRadius.circular(15),
                                boxShadow: [
                                  BoxShadow(
                                    color: Colors.blueGrey.withOpacity(0.3),
                                    blurRadius: 8,
                                    offset: const Offset(0, 4),
                                  ),
                                ],
                              ),
                              child: ElevatedButton.icon(
                                onPressed: _verileriGuncelle,
                                style: ElevatedButton.styleFrom(
                                  backgroundColor: Colors.transparent,
                                  shadowColor: Colors.transparent,
                                  shape: RoundedRectangleBorder(
                                    borderRadius: BorderRadius.circular(15),
                                  ),
                                ),
                                icon: const Icon(Icons.update_outlined,
                                    color: Colors.white),
                                label: const Text(
                                  "Güncelle",
                                  style: TextStyle(
                                    fontSize: 16,
                                    fontWeight: FontWeight.bold,
                                    color: Colors.white,
                                  ),
                                ),
                              ),
                            ),
                          ],
                        ),
                      ),
                    ),
                  ),
                ),
              ),
            ],
          ),
        ),
      ),
    );
  }
}
